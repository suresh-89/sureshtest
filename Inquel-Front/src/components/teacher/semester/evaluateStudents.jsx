...
class TeacherSemesterDirectEvaluation extends Component {
    ...
    handleSelect = (event) => {
        ...
        fetch(
            `${this.url}/teacher/subject/${this.subjectId}/semester/${this.semesterId}/direct/?username=${event.value.username}`,
            {
                method: "GET",
                headers: this.headers,
            }
                    ...
                    try {
                        if (
                            ...
                            Object.entries(
                                ...
                                // calculation total obtained marks and total marks
                                if (obtained_marks) {
                                    total_obtained_marks +=
                                        parseFloat(obtained_marks);
                                }
                                if (total_marks) {
                                    all_total_marks += parseFloat(total_marks);
                                }
    ...
    handleObtainedMarks = (event, chapter_id) => {
        let topics = this.state.topic_marks;
        let total_obtained_marks = 0;

        topics[chapter_id][event.target.name] = event.target.value;
    ...
    handleTotalMarks = (event, chapter_id) => {
        let topics = this.state.topic_marks;
        let total_marks = 0;
    ...
    handleEvaluate = () => {
        ...
        try {
            Object.entries(topics).forEach(([key, value]) => {
                ...
                value.topics.forEach((topic) => {
                    if (
                        ...
                        topic_marks[key].obtained_marks === ""
                    ) {
                        temp_topic[topic.topic_num] = {};
                    } else {
                        temp_topic[topic.topic_num] = {
                            total_marks: (
                                topic_marks[key].total_marks /
            ...
            if (this.state.selectedStudent.published === false) {
                ...
                this.setState({
                    errorMsg:
                        "You cannot evaluate a student marks once published",
                    showErrorAlert: true,
                    showLoader: false,
                });
            }
            ...
            console.log(error);
            this.setState({
                errorMsg: "Cannot evaluate marks at the moment!",
                showErrorAlert: true,
                showLoader: false,
            });
        }
            ...
            for (let i = 0; i < this.state.student_list.length; i++) {
                fetch(
                    `${this.url}/teacher/subject/${this.subjectId}/semester/${this.semesterId}/direct/publish/`,
                    {
                        ...
                        body: JSON.stringify({
                            student_username:
                                this.state.student_list[i].username,
                        }),
                    }
                )
                    .then((res) => res.json())
